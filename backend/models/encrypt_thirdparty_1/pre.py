...
def preprocess_data(pcap_directory="F:/FastAPI/backend/models/encrypt_thirdparty_1/data"):
    ...
    for pcap_file in pcap_files:
        ...
        # 处理协议列和端口列，确保数据类型正确
        df['Proto'] = df['Proto'].fillna('unknown')  # 填充缺失的协议字段
    ...
    if len(X_scaled) > sample_size:
        X_scaled, _, y, _ = train_test_split(
            X_scaled, y, train_size=100000, random_state=42, stratify=y
        )
    # ----------------
